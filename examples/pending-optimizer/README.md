...
| --- | ---: | ---: | --- |
| `tic-tac-toe-4x4.mkpro` | 156 | 105 | source port uses original-style R1/R2 counted search loops, R4..R7 line banks, source-shaped packed occupied masks, source-style `bit_or` test-and-set occupied updates, passes the player/calculator mark score into the shared line checker, uses the source-style `max` tie update for calculator move selection, keeps selected Y in real state while proving the line index is stack-only, shared indirect line update/check flow, score-derived line deltas, the original fractional win-mask report, a source-shaped physical selector for line banks, a shared `packed_score` stack helper with X-parameter-produced index accumulation, and a branchless one-based diagonal normalizer that wraps negative differences like the source; remaining blocker is optimizer/lowering size for packed line helpers |
