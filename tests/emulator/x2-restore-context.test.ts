...
describe("X2 restore context", () => {
  ...
  it("X->П immediately before ВП splices decimal hidden X2 mantissa context", () => {
    expect(runX([0x01, 0x41, 0x0c, 0x03, 0x50])).toBe("0,");
    expect(runX([0x02, 0xf0, 0x41, 0x0c, 0x03, 0x50])).toBe("0,");
    expect(runX([0x01, 0x02, 0x41, 0x0c, 0x03, 0x50])).toBe("2000,");
    expect(runX([0x01, 0x00, 0x41, 0x0c, 0x03, 0x50])).toBe("0,");
    expect(runX([0x00, 0x00, 0x41, 0x0c, 0x03, 0x50])).toBe("10000,");
    expect(runX([0x00, 0x05, 0x41, 0x0c, 0x03, 0x50])).toBe("5000,");
    expect(runX([0x01, 0x0a, 0x41, 0x0c, 0x03, 0x50])).toBe("0,");
    expect(runX([0x01, 0x0a, 0x02, 0x41, 0x0c, 0x03, 0x50])).toBe("200,");
  ...
  it("К X->П immediately before ВП splices decimal hidden X2 mantissa context", () => {
    const r7TargetsR1 = [0x01, 0x47]; // 1; X->П 7
    expect(runX([...r7TargetsR1, 0x01, 0xb7, 0x0c, 0x03, 0x50])).toBe("0,");
    expect(runX([...r7TargetsR1, 0x01, 0x02, 0xb7, 0x0c, 0x03, 0x50])).toBe("2000,");
    expect(runX([...r7TargetsR1, 0x00, 0x00, 0xb7, 0x0c, 0x03, 0x50])).toBe("10000,");
    expect(runX([...r7TargetsR1, 0x00, 0x05, 0xb7, 0x0c, 0x03, 0x50])).toBe("5000,");
    expect(runX([...r7TargetsR1, 0x01, 0x0a, 0xb7, 0x0c, 0x03, 0x50])).toBe("0,");
    expect(runX([...r7TargetsR1, 0x01, 0x0a, 0x02, 0xb7, 0x0c, 0x03, 0x50])).toBe("200,");
    expect(runX([...r7TargetsR1, 0x02, 0x0b, 0xb7, 0x0c, 0x03, 0x50])).toBe("-9000,");
    expect(runX([...r7TargetsR1, 0x00, 0x0b, 0xb7, 0x0c, 0x03, 0x50])).toBe("-1000,");
  });

  it("closed-context /-/ after X->П uses the original decimal hidden X2 mantissa for following ВП", () => {
    expect(runX([0x02, 0xf0, 0x41, 0x0b, 0x0c, 0x03, 0x50])).toBe("-2000,");
    expect(runX([0x01, 0x00, 0x41, 0x0b, 0x0c, 0x03, 0x50])).toBe("-10000,");
    expect(runX([0x00, 0x05, 0x41, 0x0b, 0x0c, 0x03, 0x50])).toBe("-5000,");
    expect(runX([0x00, 0x00, 0x41, 0x0b, 0x0c, 0x03, 0x50])).toBe("-1000,");
    expect(runX([0x02, 0x0b, 0x41, 0x0b, 0x0c, 0x03, 0x50])).toBe("2000,");
    expect(runX([0x02, 0xf0, 0x41, 0x0b, 0x0b, 0x0c, 0x03, 0x50])).toBe("2000,");
    expect(runX([0x02, 0xf0, 0x41, 0x54, 0x0b, 0x0c, 0x03, 0x50])).toBe("-2000,");
  });

  it("closed-context /-/ after К X->П uses the original decimal hidden X2 mantissa for following ВП", () => {
    const r7TargetsR1 = [0x01, 0x47]; // 1; X->П 7
    expect(runX([...r7TargetsR1, 0x02, 0xf0, 0xb7, 0x0b, 0x0c, 0x03, 0x50])).toBe("-2000,");
    expect(runX([...r7TargetsR1, 0x02, 0x0b, 0xb7, 0x0b, 0x0c, 0x03, 0x50])).toBe("2000,");
  });

  it("dot before a store-backed sign ВП restore is redundant when the sign source matches", () => {
    expect(runX([0x02, 0xf0, 0x41, 0x0a, 0x0b, 0x0c, 0x03, 0x50])).toBe(
      runX([0x02, 0xf0, 0x41, 0x0b, 0x0c, 0x03, 0x50]),
    );
    expect(runX([0x02, 0xf0, 0x41, 0x0a, 0x0b, 0x0b, 0x0c, 0x03, 0x50])).toBe(
      runX([0x02, 0xf0, 0x41, 0x0b, 0x0b, 0x0c, 0x03, 0x50]),
    );
  });
  ...
  it("delayed VP first-digit splice uses the first significant decimal source digit without sign", () => {
    expect(runX([0x03, 0x0e, 0x04, 0x11, 0x54, 0x0c, 0x02, 0x50])).toBe("100,");
    expect(runX([0x02, 0x0b, 0x0e, 0x04, 0x14, 0x54, 0x0c, 0x02, 0x50])).toBe("200,");
    expect(runX([0x00, 0x0a, 0x02, 0x0b, 0x0e, 0x04, 0x14, 0x54, 0x0c, 0x02, 0x50])).toBe("200,");
    expect(runX([0x00, 0x0b, 0x0e, 0x04, 0x14, 0x54, 0x0c, 0x02, 0x50])).toBe("0,");
  });
  ...
  it("F x^y has an exact exponent-one result on the emulator", () => {
    expect(runX([0x01, 0x0e, 0x02, 0x24, 0x50])).toBe("2,");
    expect(runX([0x01, 0x0e, 0x00, 0x0a, 0x05, 0x24, 0x50])).toBe("5,-1");
    expect(runX([0x01, 0x0e, 0x01, 0x0a, 0x02, 0x24, 0x50])).toBe("1,2");
    expect(runX([0x01, 0x0e, 0x02, 0x0b, 0x24, 0x50])).toBe("-2,");
  });

  it("scientific exact decimal values restore through X2", () => {
    expect(runX([0x08, 0x15, 0x50])).toBe("1,8");
    expect(runX([0x08, 0x15, 0x0e, 0x0a, 0x50])).toBe("1,8");
    expect(runX([0x01, 0x0c, 0x08, 0x50])).toBe("1,8");
    expect(runX([0x01, 0x0c, 0x08, 0x0e, 0x0a, 0x50])).toBe("1,8");
    expect(runX([0x01, 0x0e, 0x09, 0x09, 0x09, 0x09, 0x09, 0x09, 0x09, 0x09, 0x10, 0x0e, 0x0a, 0x50]))
      .toBe("1,8");
  });

  it("scientific X2 restores feed the next ВП as exact decimal mantissas", () => {
    expect(runX([0x08, 0x15, 0x0e, 0x0a, 0x0c, 0x02, 0x50])).toBe("1,10");
    expect(runX([0x08, 0x0b, 0x15, 0x0e, 0x0a, 0x0c, 0x02, 0x50])).toBe("1,-6");
    expect(runX([0x01, 0x0e, 0x09, 0x09, 0x09, 0x09, 0x09, 0x09, 0x09, 0x09, 0x10, 0x0e, 0x0a, 0x0c, 0x02, 0x50]))
      .toBe("1,10");
  });

  it("closed-context /-/ preserves scientific X2 display shape", () => {
    expect(runX([0x08, 0x15, 0x0e, 0x0b, 0x50])).toBe("-1,8");
    expect(runX([0x08, 0x15, 0x0e, 0x0b, 0x0c, 0x02, 0x50])).toBe("-1,10");
    expect(runX([0x08, 0x0b, 0x15, 0x0e, 0x0b, 0x50])).toBe("-1,-8");
    expect(runX([0x08, 0x0b, 0x15, 0x0e, 0x0b, 0x0c, 0x02, 0x50])).toBe("-1,-6");
  });
