...
- **x2-noop-restore** — removes `.` when the value proof shows that `X` already
  ...
  after X2-preserving gaps are only collapsed when the following command,
  possibly after free-standing empty ops, starts fresh number entry; otherwise
  the pair's X2-to-X restore is observable.
...
- **vp-x2-peephole** — drops a `К {x}` immediately after a proved `ВП`/X2
  ...
  preceding `ВП`/X2 boundary must be proved. The proof is not display-specific:
