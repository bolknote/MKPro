...
- **x2-hidden-temp-restore** — replaces a direct scratch `П->X r`, or a
  ...
  decimal source fact already in X2 through the named previous-X2-sync
  normalized restore-gap scanner. The normal stack-lift/X2-context guards still prove that
...
- **branch-target-x-reuse** — drops the first `П->X r` inside a unique branch
  ...
  value/shape jump-edge state through them; transparent helper body effects
  such as stores of the current X are replayed into the projected value/shape
  memory. X2-sensitive removal uses that projected path
