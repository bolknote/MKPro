...
- **store-recall-peephole** — drops adjacent `X->П r ; П->X r`, or a
  ...
  treats an immediate `ВП` as safe for structural hex/super recalls when the
  same structural `ВП` source is already active before the removed recall; a
  store or other context-closing command between the source sync and the recall
  keeps the recall as the visible `ВП` source. The proof also sees
...
- **last-x-reuse** — drops `П->X r` when the IR proves X already holds the
  ...
  the recall can be removed. A structural recall immediately before `ВП` can
  also be removed when the active structural `ВП` source already matches the
  recalled shape; if a store closed that source, the recall stays. Structural
