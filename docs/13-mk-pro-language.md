...
its effect checks and local static proofs allow the rewrite.
...
Only optional report metadata and machine-profile directives belong outside
`program`:
...
This language reference intentionally stops at the optimizer contract. The
concrete rewrite catalogue, capability IDs, proof model, and fixed-point IR pass
order are maintained in
[MK-Pro Optimization and Lowering Strategy Reference](20-mkpro-optimization-reference.md).

For source authors, the practical rules are:

- Write semantic MK-Pro constructs (`show`, `read`, `fn`, `match`, `loop`,
  state updates, and `raw`) rather than machine-placement hints.
- The compiler may fuse prompts, inline or share functions, reorder dispatch
  layout, reuse stack/register state, activate X2/display-byte forms, and
  select indirect or layout-sensitive machine tactics when the reportable
  proofs exist.
- `mk-pro explain` and `mk-pro --out json` are the source of truth for which
  optimizations fired, which machine features were used, and why shorter
  candidates were rejected.

The language contract is that these transformations preserve observable MK-61
behavior under the selected exact-machine model. Unsupported high-level effects
still fail compilation instead of becoming comments or implicit machine
templates.
