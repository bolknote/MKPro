...
- `sentinel-decimal-pack` — candidate that stores fixed-width non-negative counters as a leading-sentinel decimal value such as `1XXYYZZ`, so leading-zero fields remain distinguishable. For the common three two-digit field shape, the middle field can be read with a preloaded logical mask; the sentinel shifts the mask relative to the non-sentinel trick, so `1XXYYZZ` uses `800FF077` and a `10000` scale for `YY`. When a constant register is available, the scale is recalled from preload instead of emitted as `4; F10x`, matching the five-command `mask; frac; scale; *` hand form.
...
8. `return-zero-jump` — when no procedure calls are used, replaces a backward jump to `01` with `В/О` and tags it as an empty-stack optimization.
