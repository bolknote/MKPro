...
- `x2-register-dataflow` — tracks definite states of the form “X2 currently
  ...
  `same:unknown`. Whitelisted documented pure unary X computations such as
  `F sqrt`, `F x^2`, `К [x]`, and `К {x}` also seed an opaque `expr:<step>`
  value in visible X while leaving X2 untouched; a later explicit X2 sync
  (`В↑`, F* empty, direct conditional fallthrough, etc.) can then carry that
  exact computed value as a hidden temporary. Display-role, barrier,
  undocumented, dangerous, random-like, and stack-consuming commands do not
  seed such facts.
...
13. `branch-target-x-reuse` — removes the first direct or stable-indirect proved recall in a unique branch target when the source direct conditional or counted loop preserves the same recalled value in X, or when the target-entry X2 value/shape proof shows the same decimal value or structural hex/super shape is already visible. Loop-counter recalls are excluded because `F Lx` mutates that register. `С/П` counts as a no-fallthrough separator for the uniqueness check. The rewrite is refused when the target recall is needed as a `.`/`/-/`/`ВП` X2-sync boundary before the next X2-affecting op, including direct `В/О` returns, or a stack lift that can reach a downstream consumer through direct or proved-indirect flow.
