...
- `packed-line-family-layout` — detects packed banks that are both updated through `packed_add(...)` (optionally followed by the fractional report check) and scored through `packed_score(...)` across at least four distinct bank elements. The first active lowering in this family is `packed-line-family-score-tail`: for source-shaped score procedures with two direct line scores and two diagonal scores (`a+b` / `a-b`) through the same normalizer, the compiler emits one local multi-entry diagonal tail. One path calls the tail and the other falls through into it, matching the MK-61 middle-entry technique without depending on a particular procedure name. The broader shared update/check plus score walker is still reported as a considered candidate until that full layout is implemented.
