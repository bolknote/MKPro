...
- `report.sizeAttribution` — final helper/body/call-site costs and rejected
  ...
  candidates are fail-closed over the delivered CFG: a helper body ends at a
  straight-line `В/О`, and a store/recall pair is not called local when it
  crosses a branch, call, stop, return, or externally addressable entry.
  Control-crossing values remain visible under
  `valueAwareMixedStateControlCrossingNames` and are excluded from estimated
  stack-carrier savings with the normalized
  `control-crossing-state-not-stack-carrier` plan status. Stack-input
  `recall-before-store` lifetimes are likewise charged as persistent state
  updates with zero direct scheduler savings; they are never treated as
  removable temporary spills solely because both accesses occur in one helper.
  Stack-input scheduler rows also expose `valueAwareStackInputProfitBreakdown` plus the
...
- `fallthrough-jump-component-fold` is a generic final-layout block-chaining
  ...
  both jump cells. Candidate search evaluates each direct jump in its own
  transaction and temporarily excludes that pair from competing one-cell
  selector lowering: this lets the final size proof choose a two-cell fold over
  a locally attractive selector rewrite. The same proof also runs once on the
  selected final artifact without requiring any selector rewrite at all;
  changed compiler-owned preloads cause the setup program to be regenerated.
  Unrelated direct flows remain eligible for normal shortening. Raw commands,
  labelled jump/operand cells, main-entry
...
- `projected-terminal-layout-tiebreak` is a post-layout selection rule, not a
  ...
  verified candidates have equal size does the compiler run the existing
  terminal/cyclic verifier as a projection and prefer the candidate with the
  smaller independently proved projected artifact. Equal projections retain
...
- `empty-return-startup-component-transaction` composes physical-00 startup
  ...
  Startup normalization considers all direct `БП 00` edges atomically because
  the one-cell `В/О@00` cost is shared. Reachable edges qualify only when every
  authoritative execution state has an empty return stack. An edge absent from
  the complete authoritative CFG may also qualify as dead code, but the input
  CFG is first rebuilt byte-for-byte and the final CFG must prove that relocation
  introduced no path to it. This lets two independently safe edges save one cell
  where either rewrite alone would be size-neutral.
- `empty-return-tail-call-fusion` is a final-layout continuation rule applied
  inside the same physical-00 transaction. A direct or one-cell indirect call
  immediately followed by a qualifying `БП 00` becomes the corresponding tail
  jump, and the now-private continuation command is removed. Every reachable
  call state must have an empty return stack. The continuation must have no
  independent direct, indirect, external, or resumable-return entry, and the
  rewritten authoritative CFG must retain the same reachability class and
  empty-stack state at the call identity. A label between the physical cells
  is harmless only when these identity checks prove it unreferenced. The rule
  uses opcode/CFG/return-stack facts only; it does not inspect source names,
  helper names, constants, or application structure.
- `empty-return-main-edge-compaction` is the corresponding post-layout rule
  for an artifact that already has the proved empty-return trampoline
  `В/О@00 -> 01`. A direct `БП 01` becomes one `В/О` only when every complete
  authoritative execution state at that command has an empty return stack (or
  the command is absent from the complete CFG). The transaction removes only
  the direct operand cell, preserves the identity of the original command at
  physical `01`, and rebuilds direct and indirect targets, external entries,
  all return-stack states, stack/X2 state, indirect-memory facts, and setup
  preloads. It runs again after absolute/dark component layout so the two
  independent size reductions compose. No source, procedure, constant, or
  application pattern is recognized.
...
- Atomic dark-side suffix placement is evaluated for the best-ranked finalized overflow artifact regardless of which lowering flags produced it. The bounded search may place any proved straight-line helper so its last body command occupies physical `47`, add or reuse `В/О` at physical `00`, and erase the helper's explicit return at `48`; exact direct/indirect target identity, runtime selector rebinding, return-stack, stack, and X2 proofs remain mandatory, and an ordinary sibling is retained whenever the transaction does not reduce size.
...
- `return-suffix-gadget` — shares a common suffix after `return` across similar regions. It runs before continuation composition and tail-call lowering so an explicit `call; return` boundary remains available for proof. For matching direct-register `a+b` / `a-b` producers it may canonicalize subtraction as `(-b)+a`, expose one shared sequential suffix, and retain the rewrite only when the complete gadget has a strict net cell saving; unmatched subtraction is left unchanged.
...
- `shared-straight-line-helper` — turns repeated straight-line opcode runs into
  ...
  insert a helper call or `В/О` return next to an X2 restore. After leaf calls
  have been materialized as one proved indirect hole, the completed skeleton
  remains subject to the normal final-layout profitability checks; local body
  savings alone are not accepted when their labels enlarge the delivered
  program.
...
- `inline-floor-packed-row-expression` — computes floor-packed display rows inline to reduce hidden display pressure. The branch-Y/stack-SSA function-entry candidate composes this lowering with its register/stack ABI instead of testing it only as an isolated option.
...
- `logical-register-lifetime-allocation` — when ordinary source allocation runs
  ...
  precolored hardware registers, and lowers again with that assignment. At the
  final size-rescue boundary it also probes the already selected lowering even
  when ordinary allocation succeeded. Distinct logical values initialized by
  the same plain setup literal may share their entry color; any later write
  while the other value is live adds the normal interference edge and forces
  them apart. Dynamic/manual/different setup values remain mutually exclusive.
  The regenerated program is independently re-verified before it can become a
...
- `demote-constant-indirect-flow` — recompiles with selective setup-enterable
  ...
  formal address selector aliases are kept out of this demotion set. At the
  final 105-cell boundary the measured `final-layout-constant-demotion-chain`
  also follows replacement preloads through equal-size intermediate artifacts:
  a rejected link is skipped without poisoning the chain, and only a smaller
  independently compiled, static-proof-valid result can replace the incumbent.
...
- `early-helper-invariant-recall-hoist` runs the same complete-call-set and
  ...
  indirect targets follow labels; already numeric indirect targets and
  numeric/formal direct operands remain fixed and are admitted only when the
  final proof preserves their exact physical target. Moving a repeated recall
  at this stage can avoid padding that would be mandatory after addresses are
  frozen. The later post-layout form remains the fallback for candidates that
  need authoritative layout facts.
- `compiler-padding-relayout` — finalization may erase one compiler-owned `К НОП` carrying either fixed-target-hoist or natural-component-layout padding provenance. Numeric direct operands and typed indirect-flow metadata are retargeted in physical address space before rebuilding the intermediate CFG; executable address/code overlays remain immovable. The existing absolute component solver then preserves every non-retunable selector target, runtime-charged target, external entry, stack/X2 state, indirect-memory identity, and return-stack transition. The candidate is published only when the complete transaction is strictly smaller; otherwise the padding remains in place.
...
- `predecrement-indexed-stacked-value-update` preserves the indexed bank's proved physical target range on its final indirect store. Fractional/preloaded indirect-flow selectors may therefore share the resulting layout only when the final static proof shows that every annotated store target is disjoint from every selector register. Missing target metadata and real overlaps remain hard rejections; no game, bank name, or constant is recognized.
- `expression-helper-single-x-entry` is the narrow alternative to a full X/Y/Z/T helper ABI. When a shared expression reads a register value exactly once and every selected call can leave that value in X, the helper consumes X directly and omits its local `П->X`. Before emission, `expression-helper-single-x-whole-program-plan` intersects the zero-copy inputs proved at every occurrence in the main block and all procedures; it commits a primary ABI only when the observed occurrence count exactly matches the global expression-use count. Two adjacent independent assignments may be reordered so the selected value is produced last, using the same purity, cross-dependency, and previous-X-as-Y proofs as local stack forwarding. A single stored assignment immediately followed by an assignment/update can feed the same ABI directly. Persistent stores are retained unless ordinary liveness proves them dead. Uncovered, nested, or incompatible occurrences reject the global plan and retain the existing regular or mixed helper ABI, while final proved size still decides whether the candidate wins.
...
- `packed-digit-permutation-fusion` — replaces a complete width-3..8 decimal
  ...
  remains subject to normal liveness/interference proofs; incomplete,
  duplicated, mixed-source, or nonconstant mappings retain ordinary lowering.
...
14. `branch-target-x-reuse` — removes the first direct or stable-indirect proved recall in a unique branch target when the source direct conditional, counted loop, or proved stable-indirect conditional target preserves the same recalled value in X, or when the target-entry X2 register/value/shape proof shows the same register, decimal value, or structural hex/super shape is already visible. Loop-counter recalls and mutating indirect selector recalls are excluded because `F Lx`/`К x?0 R0..R6` mutate those registers. `С/П` counts as a no-fallthrough separator for the uniqueness check. Target uniqueness is resolved by the actual executable entry index, so alias labels or alternate numeric/indirect entries to the same cell keep the recall. The proof may cross free-standing stack/X2-preserving empty prefix cells, direct stores, proved stable-indirect stores, address cells, and transparent direct/proved-indirect return helpers before the target recall, carrying the branch's projected jump-edge X2 register and value/shape state through those cells; a prefix store can itself prove that the later recalled register is already visible in X. Return helpers are accepted only through the shared stack/X/X2-preserving proof, and their body value/shape effects are replayed for the target-prefix projection, so a helper that stores the incoming X can also seed the later recall proof. X2-sensitive checks are run against that projected path state, not only the globally joined target state, so unique branch entries can reuse X2/sign-source facts even when ordinary dataflow also models continuation after `С/П` or cannot follow numeric direct targets. The same numeric-target layout guard as `flow-x-reuse` is used: unresolved absolute targets keep the pass off, and deleting a branch-target recall before a later resolved numeric target is refused. The rewrite is also refused when the target recall is needed as a `.`/`/-/`/`ВП` X2-sync boundary before the next X2-affecting op, including direct `В/О` returns, or a stack lift that can reach a downstream consumer through direct or proved-indirect flow, unless the unique branch path has a previous stack-lift+X2 producer that this pass keeps and the shared scheduler proves the deeper stack difference is dead.
    `entry-stack-input-reuse` extends the same recall-removal proof across one
    direct call level. It propagates proved caller `Y/Z/T` register-value
    identities into a linearly modelled callee, intersects every stack slot at
    CFG joins, and clears facts on return. Calls without a proved target,
    unknown indirect flow, unmodelled callee operations, or observable
    recall-stack/X2 effects reject the rewrite. The `ПП`/`В/О` data-stack
    `call-entry-materialization-order` composes this proof with two other
    source-independent transformations. It may first use the existing
    helper-invariant proof to move a common recall from mixed before-call and
    after-return commutative sites into the helper tail, then reorder adjacent
    independent `П->X`/`X->П` copies so the helper's required value reaches X
    at every call. The entry recall is removed only by the ordinary bounded
    stack/X2 proof, and all three changes roll back atomically unless their
    combined IR is smaller than the original. A locally preferable root hoist
    remains the fallback when the complete composition does not win.
    contract is pinned separately by an emulator fact.
