...
- `sentinel-decimal-pack` — candidate that stores fixed-width non-negative counters as a leading-sentinel decimal value such as `1XXYYZZ`, so leading-zero fields remain distinguishable. For the common three two-digit field shape, the middle field can be read with a dual-use preloaded logical value: `1XXYYZZ` uses `100FF`, which acts as the mask and as the numeric scale for `YY`. The same value also has a useful indirect-flow side effect on MK-61-class machines: stored in a non auto-inc/dec indirect register such as `E`, it can target address `06`, so the optimizer keeps it as one hidden preload instead of spending separate `800FF077` and `10000` registers.
