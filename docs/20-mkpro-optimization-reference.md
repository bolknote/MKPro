...
- `pre-shift-stack-lift` — removes `В↑` before any proved stack-shifting
  ...
  `В↑`), after a plain X-preserving X2 sync (`F0`..`FF`), after a
  path-safe direct conditional/counting-loop fallthrough X2 sync, or after a
  ...
  and backward scheduler scans do not re-walk the same helper tree. Targeted entry labels, display/X2 restore context, stack consumers,
