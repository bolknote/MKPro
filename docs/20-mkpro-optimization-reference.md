...
25. `vp-splice` — deletes redundant exponent-entry chains (`ВП ВП`) and inert empty-op `КНОП ВП`/`К1 ВП`/`К2 ВП` forms, reporting `vp-exponent-splice` when one or more cells are removed.
