...
- `pre-shift-stack-lift` — removes `В↑` before any proved stack-shifting
  ...
  lift is only a redundant scheduler cell. Targeted entry labels, display/X2
  restore context, stack consumers, and flow commands keep the post-producer
  scan conservative.
...
- `x2-register-dataflow` — tracks definite states of the form “X2 currently
  ...
  hidden temporary. Display-role, barrier, exposing, undocumented, dangerous,
