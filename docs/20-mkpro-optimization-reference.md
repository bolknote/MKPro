...
- `expression-helper-single-x-entry` is the narrow alternative to a full X/Y/Z/T helper ABI. When a shared expression reads a register value exactly once and every selected call can leave that value in X, the helper consumes X directly and omits its local `П->X`. Two adjacent independent assignments may be reordered so the selected value is produced last; when the first reusable call has that shape, the compiler ranks both assigned values by compatible zero-copy call sites in the rest of the block and may establish the one-X ABI there instead of first emitting a regular helper entry. The same producer-to-consumer forwarding handles a single stored assignment immediately followed by an assignment/update using the helper: it calls the helper before loading the consumer's previous value, so accumulator lowering cannot destroy the proved current `X`. Persistent stores are retained unless ordinary liveness proves them dead. A helper that has already received a regular call is not converted to a partial stack ABI, and the one-X form competes with the wider stack-entry form on final proved size so local recall savings cannot override a better layout.
