...
- `x2-register-dataflow` — tracks definite states of the form “X2 currently
  ...
  stable-expression source keys; that records the source shape for hidden-temp
  equality without inferring a decimal result value. Hex and super displays are not promoted to
...
17. `x2-noop-restore` — removes `.` when X2 value dataflow proves that `X`
    ...
    source. That lets repeated structural sign toggles meet in hidden-temp
