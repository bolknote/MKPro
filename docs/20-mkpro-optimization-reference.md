...
- `stack-current-x-scheduling` — reorders current-X operations to avoid extra
  ...
  source name before it is overwritten. `stack-carried-update` is the state
  ...
  read later before being rewritten; branch consumers stay on dedicated
  guard/layout lowerings until they can be measured as post-layout candidates.
