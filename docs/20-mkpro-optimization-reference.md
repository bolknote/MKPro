...
- `branch-target-x-reuse` — avoids the first recall in a branch target when the source branch still preserves that value in X and the target has no other entry, including counted-loop targets for non-counter registers and labels separated by `С/П`, unless that recall supplies the target-side X2 sync or a stack lift that reaches a downstream consumer through direct call returns.
