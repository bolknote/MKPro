...
- All three previous-random idioms (`previous-random-stack-reuse`, `previous-random-branch-stack-reuse`, `previous-random-fractional-debit`) share one recognizer preamble (`matchPreviousRandomSeedUpdate`) and one kept-in-Y head (`emitPreviousRandomSeedUpdate`): they match `old = seed; seed = random()` — written inline or as a call to a one-statement random proc, which is inlined — and emit `recall seed; В↑; К СЧ; store seed` so the previous value stays parked in Y for the following stack-direct consumer. They differ only in how that consumer (a pure expression, a `<` branch, or a guarded fractional debit) reads Y and the new X. `show-read-debit-credit-guard` emits its negative-balance trap through the shared `emitDomainTrapOnX` tail (the same `F √` self-trap used by the store-then-domain-trap fusions).
