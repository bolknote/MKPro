...
- `store-recall-peephole` — collapses direct or stable-indirect proved
  ...
  also permits a decimal or structural recall immediately before `ВП` when the
  active decimal mantissa or structural `ВП` source already matches the recalled
  source; a store or other context-closing command keeps the recall as the
  visible source. Mutating `R0..R6` indirect
