...
| `examples/pending-optimizer/rambo-iii.mkpro` | `games/lordbss/pmk53.txt`: stops on time, then each of three front numbers; `ЕГГОГ` on bad order or fortress breach; `SOLDIER` on survival. | `show(time)`; `show(front)`; `halt("ЕГГОГ")`; `halt(7777777)` | Three-front flow is explicit; `SOLDIER` display literal is represented by a numeric marker until the display lowerer can spell it. |
