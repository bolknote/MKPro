...
| `examples/alaram.mkpro` | `games/lordbss/pmk210.txt`: `С/П@04`, `С/П@09`, `С/П@15` and terminal `В/О` paths show packed cockpit/message patterns. | `show cockpit_view intruder_mark`; terminal screens show packed patterns. | Comma removed; full cockpit video-byte composition is still represented by semantic packed fields. |
...
| `examples/dangerous-loading.mkpro` | `games/anvarov/dangerous-loading.txt`: `С/П@56` follows `Пх1; FВx; F↻; ВП; ...; /-/`, one signed fractional/video water-lane value. | `show cargo_left boat threat boats_left` | Comma removed; C/L/- lane symbols still need a real video-byte lowering. |
...
| `examples/fox-hunt-mk61.mkpro` | `games/monatkodenis/fox-hunt-mk61.txt`: `С/П@48` is the hit video value; `С/П@51` shows `R0` bearing. | `show hit_value`; `show bearing` | Matches the two-screen structure; hit is still a packed constant. |
...
| `examples/pending-optimizer/giants-country.mkpro` | `games/lordbss/pmk198.txt`: `С/П@27` shows the cave picture; `С/П@66` shows the memory challenge. | `show tile strength score`; `show warning_pattern`; `show challenge` | Comma removed; cave picture video-byte layout is not reconstructed yet. |
| `examples/pending-optimizer/labyrinth777.mkpro` | `games/anvarov/labyrinth777.txt`: `С/П@22` shows row/floor picture; `С/П@63` is a secondary stop path. | `show pos row_view energy` | Comma removed; row picture/floor packing is not reconstructed yet. |
| `examples/pending-optimizer/teleport.mkpro` | `games/anvarov/teleport.txt`: `С/П@70` and `С/П@A4` stop on packed station-state values. | `show pos danger charges loot` | Comma removed; exact packed station screen still needs source-level reconstruction. |
| `examples/pending-optimizer/tic-tac-toe-4x4.mkpro` | `games/anvarov/tic-tac-toe-4x4.txt`: `С/П@03` after `Пх2; Пх3` displays the active board/move value. | `show cell turn_count` | Comma removed; current port still shows a simplified state pair. |
| `examples/pending-optimizer/treasure-hunter-2.mkpro` | `games/anvarov/treasure-hunter-2.txt`: `С/П@16`/`С/П@97` stop on floor/plan display values. | `show pos floor_view treasure`; exit `show treasure` | Comma removed; full floor-plan screen is not reconstructed yet. |
| `examples/raja-yoga.mkpro` | `games/anvarov/raja-yoga.txt`: `С/П@37` after `Пх4; FВx; ...; ВП; 7` shows stage plus seven-cell life path. | `show stage adept goal old_age` | Comma removed; E/C/- life-path video-byte layout is not reconstructed yet. |
