...
std::optional<int> first_free_ascending(const std::set<int>& used, FeatureProfile profile);
std::optional<int> first_free_descending(const std::set<int>& used, FeatureProfile profile);
...
std::optional<int> first_free_ascending(const std::set<int>& used, FeatureProfile profile) {
  for (const int candidate : register_order_indices(profile)) {
    if (!used.contains(candidate))
      return candidate;
  }
  return std::nullopt;
...
std::optional<int> first_free_descending(const std::set<int>& used, FeatureProfile profile) {
  const auto& order = register_order_indices(profile);
...
const std::vector<int>& register_order_indices(FeatureProfile profile) {
  if (!feature_profile_has_rf_register(profile))
    return register_order_indices();
  static const std::vector<int> order = {0x0, 0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 0x7,
...
std::string register_name_for_index(int index, FeatureProfile profile) {
  if (!feature_profile_has_rf_register(profile))
    return register_name_for_index(index);
  if (index < k_min_register_index || index > feature_profile_max_register_index(profile))
    throw std::out_of_range("MK-61 register index is outside 0..f");
  if (index < 10)
    return std::to_string(index);
  return std::string(1, static_cast<char>('a' + index - 10));
}
...
std::optional<int> pick_register_index_for_variable(std::string_view variable,
  ...
  return pick_register_index_for_variable(variable, used, FeatureProfile::Standard);
}
...
std::optional<int> pick_register_index_for_variable(std::string_view variable,
  ...
  if (starts_with(variable, k_dispatch_scratch_prefix) ||
    ...
    return first_free_descending(used, profile);
  }
  ...
  return first_free_ascending(used, profile);
}

std::optional<int> pick_constant_register_index(const std::set<int>& used) {
  return pick_constant_register_index(used, FeatureProfile::Standard);
}
