...
import { compileBitSetMaskReuse, compileSegmentedBitplaneUpdateStatement, compileSingleBitMaskOpAssignment, compileSingleBitMaskOpCopyReuse, compileGridCellMaskReuse } from "./emit/lowering/spatial.ts";
...
function xParamProcParamNames(ast: ProgramAst): ReadonlySet<string> {
  ...
  const loweredProcNamesByParam = new Map<string, Set<string>>();
  for (const [procName, lowering] of lowerings) {
    const lowered = loweredProcNamesByParam.get(lowering.param) ?? new Set<string>();
    lowered.add(procName);
    loweredProcNamesByParam.set(lowering.param, lowered);
  }
  const names = new Set<string>();
  for (const [name, loweredProcNames] of loweredProcNamesByParam) {
    if (!identifierReadOutsideXParamProcs(ast, name, loweredProcNames)) names.add(name);
  }
  return names;
}

function identifierReadOutsideXParamProcs(
  ast: ProgramAst,
  name: string,
  loweredProcNames: ReadonlySet<string>,
): boolean {
  ...
  return ast.procs.some((proc) => !loweredProcNames.has(proc.name) && visitStatements(proc.body));
}
...
function inlineIndexedBitReportBranchTemp(
  ast: ProgramAst,
  assign: Extract<StatementAst, { kind: "indexed_assign" }>,
  temp: Extract<StatementAst, { kind: "assign" }>,
  branch: Extract<StatementAst, { kind: "if" }>,
): Extract<StatementAst, { kind: "if" }> | undefined {
  if (indexedPow10DeltaUpdate(assign) === undefined) return undefined;
  if (numericIndexValue(assign.target.index) !== undefined) return undefined;
  if (!expressionPureForSubstitution(assign.target.index)) return undefined;
  ...
  const report = bitAndReportForTarget(temp.expr, assign.target);
  if (report === undefined || !isSimpleStackLoad(report.mask)) return undefined;
  if (!expressionEquals(halt.expr, { kind: "identifier", name: temp.target })) return undefined;
  if (countIdentifierReadsInStatement(branch, temp.target) !== 2) return undefined;
  if (countIdentifierReadsInProgram(ast, temp.target) !== 2) return undefined;

  const fractional = fractionalTempReportCondition(branch.condition, temp.target);
  if (fractional !== undefined) {
    const condition = fractional.side === "left"
      ? {
        ...branch.condition,
        left: { kind: "call" as const, callee: "frac", args: [structuredClone(report.expr)] },
        right: structuredClone(fractional.other),
      }
      : {
        ...branch.condition,
        left: structuredClone(fractional.other),
        right: { kind: "call" as const, callee: "frac", args: [structuredClone(report.expr)] },
      };
    return {
      ...branch,
      condition,
  ...
  if (!isSimpleStackLoad(plain.neutral) || numericLiteralValue(plain.neutral) === undefined) return undefined;
...
function fractionalTempReportCondition(
  condition: ConditionAst,
  temp: string,
): { side: "left" | "right"; other: ExpressionAst } | undefined {
...
function plainTempReportCondition(
  condition: ConditionAst,
  temp: string,
): { side: "left" | "right"; neutral: ExpressionAst } | undefined {
  if (condition.op !== "!=") return undefined;
...
function isFracOfIdentifier(expr: ExpressionAst, name: string): boolean {
...
function collectSegmentedBitplaneScratchVariables(ast: ProgramAst, variables: Set<string>): void {
  ...
  const segmentedNames = new Set(
    ast.v2.state
      .filter((field) => {
        if (field.type !== "cells" || field.domain === undefined) return false;
        const board = ast.v2?.boards.find((candidate) => candidate.name === field.domain);
        return isSegmentedBitplaneBoard(board);
      })
...
function isReusableBitSetPair(
  ...
  if (firstSet !== undefined && secondSet !== undefined) {
    return expressionEquals(firstSet.item, secondSet.item);
  }
  const firstMaskSet = matchBitOrMaskAssignmentForReuse(first);
  const secondMaskSet = matchBitOrMaskAssignmentForReuse(second);
  return firstMaskSet !== undefined &&
    secondMaskSet !== undefined &&
    expressionEquals(firstMaskSet.mask, secondMaskSet.mask);
}

function matchBitOrMaskAssignmentForReuse(
  statement: Extract<StatementAst, { kind: "assign" }>,
): { mask: ExpressionAst } | undefined {
  const expr = statement.expr;
...
const optimizerCapabilities: Array<{
  ...
  },
  {
    ...
    detail: "Uses the hidden X2 display register as a temporary across ordinary X2-preserving logic: a membership test can keep a deterministic mask in Y, direct scratch recalls and repeated literals can restore a proved X2-resident value through '.' after either a safe restore gap, a CFG-proved immediate X2 sync, or a modeled closed-context /-/ dot source, and the shared ВП restore-gap proof can cross transparent direct/proved-indirect return helpers before deciding that '.' or a literal restore preserves the same mantissa source; restored dots keep hidden reg:r value facts alive for later scratch aliases, direct stores remember proved decimal register contents so later recalls can rehydrate dot-safe decimal and ВП splice facts, no-op restores are removed when value dataflow proves X already contains the hidden X2 value, and safe decimal restore results are removed when a following hard X/X2 overwrite such as Cx makes the restored X dead; a separate mantissa/exponent shape lattice tracks active ВП forms without granting dot-safe value aliases until a closing X2 sync normalizes them; known-fractional masks insert a preserving gap before the restore when К{x} is skipped.",
  },
  {
