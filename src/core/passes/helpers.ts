...
export interface X2MantissaVpSpliceModel {
  readonly decimal?: X2MantissaDataModel | undefined;
  readonly structural?: X2MantissaDataModel | undefined;
}
export interface X2MantissaFirstDigitSpliceModel extends X2MantissaVpSpliceModel {}
...
function structuralAbsDecimalValue(fact: X2ShapeFact): string | undefined {
  ...
  return abs === undefined ? undefined : x2ShapeFactExactNonNegativeDisplayDecimal(abs);
}
...
function structuralHexSquareDecimalProduct(
  ...
  const model = x2ShapeDataModelForFact(fact);
  const scaledZero = structuralScaledHexZeroSquareDecimalProduct(model);
  if (scaledZero !== undefined) return scaledZero;
...
function structuralScaledHexZeroSquareDecimalProduct(
  model: X2ShapeDataModel,
): StructuralHexDecimalProduct | undefined {
  const operand = structuralScaledHexZeroSquareOperandFromShapeModel(model);
  if (operand === undefined) return undefined;
  if (operand.digit === 10) return structuralScaledAHexSquareDecimalProduct(operand.exponent);
  return { value: "0", display: "0" };
}

function structuralScaledAHexSquareDecimalProduct(exponent: number): StructuralHexDecimalProduct | undefined {
  if (!Number.isInteger(exponent)) return undefined;
  if (exponent >= 0) return { value: "0", display: "0".repeat(exponent * 2 + 2) };
  const displayExponent = canonicalExponentShapeRaw(String(exponent * 2 + 1));
  return displayExponent === undefined
    ? undefined
    : { value: "0", displayShape: decimalExponentShapeFact("0", displayExponent) };
}

function structuralScaledHexZeroSquareOperandFromShapeModel(
  model: X2ShapeDataModel,
  ...
  const raw = model.sign === "" ? model.canonical : model.canonical.slice(1);
  const integer = /^0*([ЕEF])(0+)$/u.exec(raw);
  if (integer !== null) {
...
function isVerifiedScaledHexZeroSquareDigit(digit: number): boolean {
...
function plainProducesStructuralBinaryDecimalValues(
  ...
  const output = new Set<X2ValueFact>();
  for (const value of structuralHexBinaryDecimalValues(op, y, x, yShape, xShape, directYShape, directXShape)) {
    output.add(decimalValueFact(value, "normalized"));
  }
  return output;
}
...
function structuralHexBinaryDecimalValues(
  ...
  const output = new Set<string>();
  for (const product of structuralHexBinaryDecimalProducts(op, y, x, yShape, xShape, directYShape, directXShape)) {
...
function structuralHexBinaryDecimalProducts(
  op: Extract<IrOp, { kind: "plain" }>,
  y: X2ValueSet | undefined,
  x: X2ValueSet | undefined,
  yShape: X2ShapeSet | undefined,
...
function addStructuralHexDecimalProduct(
  output: StructuralHexDecimalProduct[],
...
type StructuralHexArithmeticOperand =
  ...
  | { readonly kind: "carry-normalized"; readonly product: StructuralHexDecimalProduct };

interface StructuralHexArithmeticOperandOptions {
  readonly carryShapes?: X2ShapeSet | undefined;
}

function structuralHexArithmeticOperands(
  shapes: X2ShapeSet | undefined,
  options: StructuralHexArithmeticOperandOptions = {},
): StructuralHexArithmeticOperand[] {
...
function structuralHexCarryNormalizedOperandKey(product: StructuralHexDecimalProduct): string {
...
function structuralHexOperandMinusDecimalProduct(
  ...
  switch (left.kind) {
    ...
    case "carry-normalized":
      return structuralHexCarryNormalizedBinaryDecimalProduct(left.product, right, 0x11);
  }
}
...
function decimalMinusStructuralHexOperandProduct(
  ...
  switch (right.kind) {
    ...
    case "carry-normalized":
      return decimalBinaryStructuralHexCarryNormalizedProduct(left, right.product, 0x11);
  }
}
...
function structuralHexOperandTimesDecimalProduct(
  ...
  switch (left.kind) {
    ...
    case "carry-normalized":
      return structuralHexCarryNormalizedBinaryDecimalProduct(left.product, right, 0x12);
  }
}
...
function decimalTimesStructuralHexOperandProduct(
  ...
  switch (right.kind) {
    ...
    case "carry-normalized":
      return decimalBinaryStructuralHexCarryNormalizedProduct(left, right.product, 0x12);
  }
}
...
function structuralHexOperandDivideDecimalProduct(
  ...
  switch (left.kind) {
    ...
    case "carry-normalized":
      return structuralHexCarryNormalizedBinaryDecimalProduct(left.product, right, 0x13);
  }
}
...
function decimalDivideStructuralHexOperandProduct(
  ...
  switch (right.kind) {
    ...
    case "carry-normalized":
      return decimalBinaryStructuralHexCarryNormalizedProduct(left, right.product, 0x13);
  }
}
...
function structuralHexCarryNormalizedPlusDecimalProduct(
  ...
  return structuralHexCarryNormalizedBinaryDecimalProduct(left, right, 0x10);
}

function structuralHexCarryNormalizedBinaryDecimalProduct(
  left: StructuralHexDecimalProduct,
  right: string,
  opcode: 0x10 | 0x11 | 0x12 | 0x13,
): StructuralHexDecimalProduct | undefined {
  if (!/^[0-9]+$/u.test(left.value)) return undefined;
  if (parseExactDecimal(right) === undefined) return undefined;
  return structuralHexDecimalProductFromConcreteDecimalValue(
    concreteDecimalBinaryValue(opcode, left.value, right),
  );
}

function decimalBinaryStructuralHexCarryNormalizedProduct(
  left: string,
  right: StructuralHexDecimalProduct,
  opcode: 0x10 | 0x11 | 0x12 | 0x13,
): StructuralHexDecimalProduct | undefined {
  if (!/^[0-9]+$/u.test(right.value)) return undefined;
  if (parseExactDecimal(left) === undefined) return undefined;
  return structuralHexDecimalProductFromConcreteDecimalValue(
    concreteDecimalBinaryValue(opcode, left, right.value),
  );
}

function structuralHexDecimalProductFromConcreteDecimalValue(
...
const STRUCTURAL_HEX_DIGIT_TIMES_DECIMAL_TABLE: ReadonlyMap<string, StructuralHexDecimalProduct> = new Map([
  ...
  ["10:10", { value: "0", display: "00" }],
  ["10:11", { value: "10", display: "10" }],
  ["10:12", { value: "4", display: "04" }],
  ["10:13", { value: "14", display: "14" }],
  ["10:14", { value: "8", display: "08" }],
  ["10:15", { value: "990", display: "990" }],
  ["10:16", { value: "0", display: "000" }],
  ["10:17", { value: "10", display: "010" }],
  ...
  ["11:10", { value: "10", display: "10" }],
  ["11:11", { value: "21", display: "21" }],
  ["11:12", { value: "16", display: "16" }],
  ["11:13", { value: "11", display: "11" }],
  ["11:14", { value: "22", display: "22" }],
  ["11:15", { value: "21", display: "021" }],
  ["11:16", { value: "32", display: "032" }],
  ["11:17", { value: "43", display: "043" }],
  ...
  ["12:10", { value: "20", display: "20" }],
  ["12:11", { value: "32", display: "32" }],
  ["12:12", { value: "28", display: "28" }],
  ["12:13", { value: "24", display: "24" }],
  ["12:14", { value: "20", display: "20" }],
  ["12:15", { value: "52", display: "052" }],
  ["12:16", { value: "904", display: "904" }],
  ["12:17", { value: "900", display: "900" }],
  ...
  ["13:10", { value: "30", display: "30" }],
  ["13:11", { value: "43", display: "43" }],
  ["13:12", { value: "40", display: "40" }],
  ["13:13", { value: "53", display: "53" }],
  ["13:14", { value: "50", display: "50" }],
  ["13:15", { value: "923", display: "923" }],
  ["13:16", { value: "920", display: "920" }],
  ["13:17", { value: "933", display: "933" }],
  ...
  ["14:10", { value: "40", display: "40" }],
  ["14:11", { value: "54", display: "54" }],
  ["14:12", { value: "52", display: "52" }],
  ["14:13", { value: "50", display: "50" }],
  ["14:14", { value: "4", display: "4" }],
  ["14:15", { value: "922", display: "922" }],
  ["14:16", { value: "920", display: "920" }],
  ["14:17", { value: "934", display: "934" }],
...
function structuralHexDigitTimesStructuralHexDigitProduct(
  leftDigit: number,
  rightDigit: number,
): StructuralHexDecimalProduct | undefined {
  if (!isVerifiedArithmeticHexDigit(leftDigit) || !isVerifiedArithmeticHexDigit(rightDigit)) return undefined;
  if (rightDigit === 14) return { value: "0", display: "0" };
  const value = exactDecimalToNormalized(BigInt((leftDigit - 10) * 10), 0);
  if (value === undefined) return undefined;
...
function structuralHexDigitDivideDecimalProduct(
  leftDigit: number,
  right: string,
): StructuralHexDecimalProduct | undefined {
  if (!isVerifiedArithmeticHexDigit(leftDigit)) return undefined;
  return STRUCTURAL_HEX_DIGIT_DIVIDE_DECIMAL_TABLE.get(`${leftDigit}:${right}`);
}
...
const DECIMAL_DIVIDE_STRUCTURAL_HEX_DIGIT_TABLE: ReadonlyMap<string, StructuralHexDecimalProduct> = new Map([
  ...
  ["10:11", { value: "9.099099", display: "9.099099" }],
  ["10:12", { value: "9.9099099", display: "9.9099099" }],
  ["10:13", { value: "9.9099099", display: "9.9099099" }],
  ["10:14", { value: "9.9099099", display: "9.9099099" }],
  ["11:11", { value: "9.099099", display: "9.099099" }],
  ["11:13", { value: "9.8", display: "9.8" }],
  ["11:14", { value: "9.0292929", display: "9.0292929" }],
  ["12:11", { value: "9.099099", display: "9.099099" }],
  ["12:13", { value: "0", display: "0" }],
  ["12:14", { value: "9.3292929", display: "9.3292929" }],
  ["13:11", { value: "9.099099", display: "9.099099" }],
  ["13:13", { value: "0.2", display: "0.2" }],
  ["13:14", { value: "9.6292929", display: "9.6292929" }],
  ["14:11", { value: "9.099099", display: "9.099099" }],
  ["14:13", { value: "0.4", display: "0.4" }],
  ["14:14", { value: "9.9292929", display: "9.9292929" }],
  ["15:11", { value: "9", display: "9" }],
  ["15:13", { value: "9", display: "9" }],
  ["15:14", { value: "0.2292929", display: "0.2292929" }],
  ["16:11", { value: "9.2525252", display: "9.2525252" }],
  ["16:13", { value: "9.2", display: "9.2" }],
  ["16:14", { value: "0.5292929", display: "0.5292929" }],
  ["17:11", { value: "9.3434343", display: "9.3434343" }],
  ["17:13", { value: "9.4", display: "9.4" }],
  ["17:14", { value: "9.2292929", display: "9.2292929" }],
...
function decimalMinusStructuralHexExponentProductFromPinnedOperand(
  ...
  let result: bigint;
  if (right.digit === 10) {
    result = BigInt(left) * pow10BigInt(operand.scale) - operand.num;
    return structuralHexDecimalProductFromExact(result, operand.scale);
  }
...
function structuralHexDecimalProductFromExact(
  ...
  const value = exactDecimalToNormalized(num, scale);
  if (value === undefined) return undefined;
  const displayShape = exactDecimalDisplayShapeFact(value);
  return displayShape === undefined ? undefined : { value, displayShape };
}

function structuralHexExponentDivideDecimalProduct(
  left: StructuralHexExponentOperand,
  right: string,
): StructuralHexDecimalProduct | undefined {
  if (!isVerifiedArithmeticHexDigit(left.digit) || !isVerifiedScaledStructuralHexExponent(left.exponent)) {
...
): ReadonlyMap<string, StructuralHexDecimalProduct> {
  const output = new Map<string, StructuralHexDecimalProduct>();
  for (const [key, display] of entries) {
    const product = structuralHexDecimalProductFromMk61Display(display);
    if (product === undefined) throw new Error(`Invalid MK-61 display product ${display}`);
    output.set(key, product);
  }
  return output;
}

function structuralHexDecimalProductFromMk61Display(display: string): StructuralHexDecimalProduct | undefined {
  const match = /^(-?)([0-9]+)(?:,([0-9]*))?([+-]?[0-9]{2})?$/u.exec(display);
  if (match === null) return undefined;
  const sign = match[1]!;
  const integer = match[2]!;
  const fraction = match[3];
  const exponentRaw = match[4];
  const mantissa = fraction === undefined || fraction.length === 0
    ? `${sign}${integer}`
    : `${sign}${integer}.${fraction}`;
  const value = exponentRaw === undefined
    ? normalizePlainDecimal(mantissa)
    : normalizedMk61DisplayExponentValue(mantissa, exponentRaw);
  if (value === undefined) return undefined;
  if (exponentRaw !== undefined) {
    const exponent = canonicalMk61DisplayExponentRaw(exponentRaw);
    if (exponent === undefined) return undefined;
    return {
      value,
      displayShape: decimalExponentShapeFact(mantissa, exponent),
    };
  }
  return { value, display: mantissa };
}

function normalizedMk61DisplayExponentValue(mantissa: string, exponentRaw: string): string | undefined {
  const mantissaParts = parseExactDecimal(mantissa);
  const canonicalExponent = canonicalMk61DisplayExponentRaw(exponentRaw);
  const exponent = canonicalExponent === undefined ? NaN : Number(canonicalExponent);
  if (mantissaParts === undefined || !Number.isInteger(exponent)) return undefined;
  const shiftedScale = mantissaParts.scale - exponent;
  const num = shiftedScale < 0 ? mantissaParts.num * pow10BigInt(-shiftedScale) : mantissaParts.num;
  return exactDecimalToNormalized(num, Math.max(0, shiftedScale));
}

function canonicalMk61DisplayExponentRaw(raw: string): string | undefined {
  const canonical = canonicalExponentShapeRaw(raw);
  if (canonical === undefined) return undefined;
  const sign = canonical.startsWith("-") ? "-" : "";
  const digits = sign === "" ? canonical : canonical.slice(1);
  const stripped = digits.replace(/^0+/u, "") || "0";
  return stripped === "0" ? "0" : `${sign}${stripped}`;
}
...
function structuralBitwiseDecimalValues(
  op: Extract<IrOp, { kind: "plain" }>,
  y: X2ValueSet | undefined,
  x: X2ValueSet | undefined,
  yShape: X2ShapeSet | undefined,
  ...
  for (const leftOperand of left) {
    for (const rightOperand of right) {
      const result = structuralBitwiseDecimalValue(op.opcode, leftOperand, rightOperand);
      if (result !== undefined) output.add(result);
    }
  }
  return output;
...
function plainXValueAfterNonPreservingOp(
  ...
  const output = plainProducesStableExpressionValues(op, x, y, xShape, yShape, directYShape, directXShape);
...
export function x2ShapeSetHasExactNonNegativeIntegerDisplay(input: X2ShapeSet | undefined): boolean {
  for (const fact of input ?? []) {
    if (!x2ShapeFactHasExactIntegerDisplay(fact)) continue;
    const decimal = x2ShapeFactRestoredVisibleDecimal(fact);
    if (decimal !== undefined && !decimal.startsWith("-")) return true;
  }
  return false;
}
...
function x2ShapeFactHasExactNonNegativeDisplay(fact: X2ShapeFact): boolean {
  return x2ShapeFactExactNonNegativeDisplayDecimal(fact) !== undefined;
}
...
export function x2ValueShapeSetRestoredVisibleDecimals(
  ...
  for (const decimal of x2ShapeSetRestoredVisibleDecimals(shapeSetWithStableExpressionValueShapes(shapes, values))) {
    output.add(decimal);
  }
  return output;
}
...
export function x2MantissaStoreVpSpliceModel(
  source: X2MantissaDataModel,
): X2MantissaVpSpliceModel | undefined {
  const decimal = x2DecimalStoreVpSpliceModel(source);
  const structural = x2StructuralStoreVpSpliceModel(source);
  return decimal === undefined && structural === undefined ? undefined : { decimal, structural };
}

export function x2DecimalStoreVpSpliceModel(source: X2MantissaDataModel): X2MantissaDataModel | undefined {
  if (source.radix !== "decimal") return undefined;
  const spliced = decimalStoreVpSpliceMantissa(source.canonical);
  return spliced === undefined ? undefined : decimalMantissaDataModel(spliced);
}

export function x2StructuralStoreVpSpliceModel(source: X2MantissaDataModel): X2MantissaDataModel | undefined {
  return x2StructuralMantissaDropFirstDigitShapeModel(source);
}

export function x2MantissaIndirectFlowVpSpliceModel(
  source: X2MantissaDataModel,
): X2MantissaVpSpliceModel | undefined {
  const decimal = x2DecimalIndirectFlowVpSpliceModel(source);
  const structural = x2StructuralIndirectFlowVpSpliceModel(source);
  return decimal === undefined && structural === undefined ? undefined : { decimal, structural };
}

export function x2DecimalIndirectFlowVpSpliceModel(source: X2MantissaDataModel): X2MantissaDataModel | undefined {
  const spliced = indirectFlowVpSpliceMantissa(source);
  return spliced === undefined ? undefined : decimalMantissaDataModel(spliced);
}

export function x2StructuralIndirectFlowVpSpliceModel(source: X2MantissaDataModel): X2MantissaDataModel | undefined {
  if (source.radix !== "hex" && source.radix !== "super") return undefined;
  if (source.digits.length === 0) return undefined;
...
export function recallAlreadySyncedInX2RestoredVisibleDecimal(
  ...
  for (const decimal of dotSafeDecimalShapeValues(effectiveX2StateShape(state))) hiddenDecimals.add(decimal);
...
export function recallAlreadySyncedInX2StructuralShape(
  ...
  return x2ShapeSetsHaveSameStructuralShape(effectiveX2StateShape(state), shapes) ? register : undefined;
}
...
export function recallAlreadySyncedInX2VpShape(
  ...
  return x2ShapeSetsHaveSameRestoredDisplayShape(effectiveX2StateShape(state), shapes) ? register : undefined;
}
...
export function recallAlreadyInXStructuralShape(
  ...
  if (register === undefined || state === undefined) return undefined;
  const shapes = recallStructuralShapeFacts(op, state, register);
  return x2ShapeSetsHaveSameStructuralShape(effectiveVisibleXStateShape(state), shapes) ? register : undefined;
}
...
export function recallAlreadyInXRestoredDisplayShape(
  ...
  if (register === undefined || state === undefined) return undefined;
  const shapes = recallVpEntryShapeSourceFacts(op, state, register);
  return x2ShapeSetsHaveSameRestoredDisplayShape(effectiveVisibleXStateShape(state), shapes) ? register : undefined;
}
...
export function recallAlreadyInXDecimalDisplayShape(
  ...
  if (register === undefined || state === undefined) return undefined;
  const shapes = recallDecimalDisplayShapeFacts(op, state, register);
  return x2ShapeSetsHaveSameDecimalDisplayShape(effectiveVisibleXStateShape(state), shapes) ? register : undefined;
}
...
function transferPlainX2ValueState(
  ...
  if (closedExponentValues.size > 0) {
    ...
    const x = plainPreservesXValue(op)
      ? new Set(sourceX)
      : plainXValueAfterNonPreservingOp(
        op,
        producerIndex,
        sourceX,
        input.y,
...
function transferConditionalX2VpEntryShapeState(
  ...
  return effect === "affects" ? sharedStructuralShapeFacts({ x, x2: x, xShape, x2Shape }) : undefined;
}
...
function vpEntryShapesFromStoreSplice(shapes: X2ShapeSet | undefined): X2ShapeSet | undefined {
  ...
  for (const fact of structuralRestoreShapeFacts(canonicalStructuralShapeFacts(shapes))) {
    ...
    const shape = spliced === undefined ? undefined : x2MantissaShapeFactFromModel(spliced);
    if (shape !== undefined) output.add(shape);
  }
  return output.size === 0 ? undefined : output;
}
...
function signChangedClosedShapeMantissas(input: X2ValueDataflowState): ReadonlySet<string> | undefined {
  const mantissas = new Set<string>();
  const xShape = shapeSetWithStableExpressionValueShapes(input.xShape, input.x);
  const x2Shape = shapeSetWithStableExpressionValueShapes(input.x2Shape, input.x2);
  for (const fact of x2Shape ?? []) {
    const model = x2ShapeDataModelForFact(fact);
    if (model.kind !== "mantissa" || model.radix !== "decimal" || model.safety !== "dotSafeDecimal") continue;
    ...
    if (!xValueOrShapeCanFeedClosedDecimalSignChange(input.x, xShape, fact, normalized)) continue;
...
function signChangedClosedDecimalExponentShapeState(
  ...
  for (const fact of x2Shape ?? []) {
    ...
    if (!hasSharedValue) {
      const keys = sharedRestoredDisplaySourceKeys(xShape, new Set([fact]));
...
function signChangedClosedStructuralShapeFacts(input: X2ValueDataflowState): ReadonlySet<X2ShapeFact> | undefined {
  const shapes = x2SignChangedSharedStructuralShapeFacts(
    shapeSetWithStableExpressionValueShapes(input.xShape, input.x),
    shapeSetWithStableExpressionValueShapes(input.x2Shape, input.x2),
  );
  return shapes.size === 0 ? undefined : shapes;
}
...
function signChangedClosedStructuralState(
  ...
  for (const key of sharedStructuralRestoreSourceKeys(
    shapeSetWithStableExpressionValueShapes(input.xShape, input.x),
    shapeSetWithStableExpressionValueShapes(input.x2Shape, input.x2),
  )) {
    values.add(stableExpressionValueFact("0B", key));
...
function sharedExactDecimalDisplayShapeFacts(
  ...
  for (const fact of decimalDisplayShapeFacts(x2Shape)) {
    const visible = x2ShapeFactRestoredVisibleDecimal(fact);
  ...
  for (const fact of decimalDisplayShapeFacts(xShape)) {
    const visible = x2ShapeFactRestoredVisibleDecimal(fact);
    if (visible === undefined) continue;
    const sourceValue = decimalValueFact(visible, "normalized");
...
function sharedDecimalVpEntryMantissas(
  ...
  const mantissas = new Set<string>();
  const signedZero = sharedSignedZeroDecimalMantissas(effectiveInputXShape(input), effectiveInputX2Shape(input));
